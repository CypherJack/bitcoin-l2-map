...
import { NODES, EDGES, CATEGORIES, TIER_Y, state, stage, edgesSvg } from './data.js';
import { syncHighlight } from './interaction.js';
...
export function drawEdges() {
  ...
  if (!btcPx || !lnPx) { syncHighlight(); return; }

  const slabHalfW = width * 0.4;  // 80% width → half
  const slabHalfH = 28;
  const Y_NP = 0.37 * height;     // native primitives row
  const nodeR = 42;

  // Label zones — y-band between strata where labels should sit
  ...
  // Slab side: angle the line so it is perpendicular for nodes above the
  // slab center and up to ±20° at the slab edges. Node side: line starts
  // from node center (hidden under the icon).
  const MAX_TILT_BTC = 20 * Math.PI / 180;
  const MAX_TILT_LN  = 10 * Math.PI / 180;
  function attach(id, otherId) {
    const p = state.px[id];
  ...
  // Build items: geometry + category color
  const items = [];
  EDGES.forEach((e) => {
  ...
  // Create label DOM elements (position set later via shelf placement)
  items.forEach((it) => {
  ...
  // Shelf-based placement inside each zone: labels sit on horizontal shelves,
  // each pinned to where its line crosses the shelf y — staying ON the connection.
  Object.entries(ZONE).forEach(([zkey, Z]) => {
    ...
    zItems.sort((a, b) => a._sortX - b._sortX);

    const shelfOccupants = shelfYs.map(() => []);
    zItems.forEach((it) => {
      let placed = false;
      for (let s = 0; s < shelfCount; s++) {
        ...
        const leftX = lx - it.halfW, rightX = lx + it.halfW;
  ...
  items.forEach((it) => {
    it.g.setAttribute('transform', `translate(${it.labelX},${it.labelY})`);
  });
